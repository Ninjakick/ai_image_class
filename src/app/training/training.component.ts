...
export class TrainingComponent implements OnInit {
  ...
  public displayedColumns: string[] = ['ImageSrc','Class','Label_X1','Label_X2','Label_X3'];
  
  private csvContent: any;

  private label_x1: number[]=[];
  private label_x2: number[]=[];
  private label_x3: number[]=[];
  ...
  async train()
  { 
    const {images, targets} = this.generateData(this.csvContent,138);
  ...
  async getModifiedMobilenet()
  {
    ...
    const predictions = <tf.SymbolicTensor> tf.layers.dense({units: 3, activation: 'softmax',name: 'denseModified'}).apply(x.output);
  ...
  parseImages(batchSize)
  {
    ...
    for ( let i = 0; i < batchSize; i++) 
    {
    	if (allTextLines[i] != undefined) {
			...
			this.tableRows.push({ImageSrc: '', LabelX1: 0 , LabelX2: 0, LabelX3: 0 ,Class: ''});
          ...
          this.tableRows[i].LabelX3=0;
				  this.tableRows[i].Class="plusieur";
				} 
        if (cols[1]=="aucun") 
        { 
          this.label_x1.push(Number('0'));
          this.label_x2.push(Number('0'));
          this.label_x3.push(Number('1'));
          this.tableRows[i].ImageSrc="../assets/"+ cols[0];
          this.tableRows[i].LabelX1=0;
          this.tableRows[i].LabelX2=0;
          this.tableRows[i].LabelX3=1;
          this.tableRows[i].Class="aucun";
        } 
    ...
    for ( let i = 0; i < batchSize; i++) 
    {
    	if (allTextLines[i] != undefined) {
	      ...
	      if (cols[0].split(csvSeparator_2)[1]=="jpg") 
	      {
	        ...
	        let targetTensor =tf.tensor1d([this.label_x1[i],this.label_x2[i],this.label_x3[i]]);
  ...
  getTotalAucun() 
  {
    return this.tableRows.map(t => t.LabelX3).reduce((acc, value) => acc + value, 0);
  };
